...
export function inizializzaUI() {
    ...
    const btnCatturaFotocamera = document.getElementById('btn-cattura-fotocamera');
    ...
    async function apriFotocamera() {
        if (!window.isSecureContext || !navigator.mediaDevices?.getUserMedia) {
            mostraToast('Per scattare dalla fotocamera apri l’app da un indirizzo HTTPS.');
            return;
        }

        try {
            streamFotocamera = await navigator.mediaDevices.getUserMedia({
                video: { facingMode: { ideal: 'user' }, width: { ideal: 1600 }, height: { ideal: 1600 } },
                audio: false
            });
            ...
            mostraToast('Impossibile aprire la fotocamera. Controlla i permessi del browser.');
        }
    }
    ...
    btnCatturaFotocamera?.addEventListener('click', async () => {
        ...
        chiudiFotocamera();
        if (blob) gestisciFileSelezionato(new File([blob], 'foto-scattata.jpg', { type: 'image/jpeg' }));
    });
    ...
    async function gestisciFileSelezionato(file) {
        ...
        try {
            ...
            mostraToast('Impossibile leggere questa foto. Riprova con un file JPEG o PNG.');
        }
    ...
    async function ottimizzaFotoPerAnalisi(file) {
        ...
        try {
            ...
            const scala = Math.min(1, latoMassimo / Math.max(immagine.naturalWidth, immagine.naturalHeight));
            const canvas = document.createElement('canvas');
...
function immagineFallback(img) {
    ...
    img.src =
        '/images/haircuts/placeholder.jpg';
...
function creaCardTaglio(taglio, indice) {
    ...
    return `
        ...
        >
            ...
            <div class="taglio-card-immagine">

                <img
                    src="${immagine}"
                    alt="${nome}"
                    loading="lazy"
                    onerror="this.onerror=null; this.src='Copilot_20260728_144629.png';"
                >
...
export function mostraRisultatiAnalisi(dati) {
    ...
    let tagli =
        Array.isArray(dati.tagli_consigliati)
    ...
    */
    ...
    cardContainer.innerHTML = `
        ...
        <div class="tabs-header">
            ...
            <button
                class="tab-btn"
                onclick="cambiaTab('tab-visagismo')"
            >
                👤 Visagismo
            </button>
            ...
            <button
                class="tab-btn"
                onclick="cambiaTab('tab-parrucchiere')"
            >
                💈 Parrucchiere
            </button>
        ...
        <div class="trova-parrucchiere-section">
            ...
            <div class="trova-parrucchiere-content">
                ...
                <span class="trova-parrucchiere-icon">
                    📍
                </span>
        ...
        .forEach(button => {
            ...
            button.addEventListener(
                ...
                () => {
                    ...
                    document.getElementById('tab-taglio')
                    ?.scrollIntoView({
                         behavior: 'smooth',
                            block: 'start'
                        });
    ...
    document .getElementById('btn-trova-parrucchiere' )?.addEventListener('click',() => {
      trovaParrucchiere();
            }
        );
...
function creaDettaglioTaglio(taglio) {
    ...
    return `
        ...
        <section class="anteprima-taglio" style="margin-top:24px;">
            ...
            <button type="button" class="bottone" id="btn-genera-anteprima"
                data-nome-taglio="${taglio.nome || ''}"
                data-descrizione-taglio="${taglio.descrizione || ''}">
...
function inizializzaTransizioniNav() {
    ...
    if (!overlay || linkNav.length === 0) {
        ...
        console.warn(
            'Transizioni nav: overlay o link non trovati',
            {
                overlay,
                numeroLink:
                    linkNav.length
